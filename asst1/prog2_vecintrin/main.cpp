...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask maskWork, maskIsNegative, maskIsNotNegative, maskTemp;

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    int vecwidth=N-i>=VECTOR_WIDTH?VECTOR_WIDTH:N-i;		
    maskWork = _cs149_init_ones(vecwidth); 					//Set remaining width
    maskIsNegative = _cs149_init_ones(0);
	maskTemp = _cs149_init_ones(0);

    // Load vector of values from contiguous memory addresses
    _cs149_vload_float(x, values+i, maskWork);              // x = values[i];
    _cs149_vload_int(y, exponents+i, maskWork);				// y = exponents[i];

	_cs149_veq_int(maskTemp, y, zeroint, maskWork);			// if (y == 0) {
	_cs149_vset_float(result, 1.f, maskTemp);				// output[i] = 1.f;

	maskTemp=_cs149_mask_not(maskTemp);						// } else {
	maskTemp=_cs149_mask_and(maskTemp, maskWork);
	__cs149_mask maskTemp1=maskTemp;

	_cs149_vload_float(result, values+i, maskTemp);  		// float result = x;
	_cs149_vsub_int(y, y, oneint, maskTemp);				// int count = y - 1;
	_cs149_vgt_int(maskTemp, y, zeroint, maskTemp);

	while (_cs149_cntbits(maskTemp)){						// while (count > 0) {
		_cs149_vmult_float(result, result, x, maskTemp);	// result *= x;
		_cs149_vsub_int(y, y, oneint, maskTemp);			// count--;
		_cs149_vgt_int(maskTemp, y, zeroint, maskTemp);
	}

	_cs149_vgt_float(maskTemp, result, clamp, maskTemp1);	// if (result > 9.999999f) {
	_cs149_vset_float(result, 9.999999f, maskTemp);			// result = 9.999999f;

    _cs149_vstore_float(output+i, result, maskWork);		// output[i] = result;
  }
...
float arraySumVector(float* values, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {				// O(N/VECTOR_WIDTH) sum vectors of vector width
	_cs149_vload_float(x, values+i, maskAll);
	_cs149_vadd_float(result, result, x, maskAll);
  }
  for (int j=1;j<VECTOR_WIDTH;j*=2){				// O(log2(VECTOR_WIDTH)) addition
